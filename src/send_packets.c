...
static void calc_sleep_time(tcpreplay_t *ctx, struct timespec *pkt_time, //PB:
...
{   
    ...
    struct timespec now, print_delta; // PB
    tcpreplay_opt_t *options = ctx->options;
    ...
    ctx->timefunction.gettime(&now); // PB: calc_gettime
...
#error WTF???  We should not be here!
        ...
        if (skip_length && pktlen < skip_length) {
            ...
            if (!top_speed) {
                now_is_now = true;
                ctx->timefunction.gettime(&now); //PB: to use nano and microsecond accuracy
                // gettimeofday(&now, NULL);
            }
        ...
        if (options->stats > 0) {
            if (! timesisset(&stats->last_print)) { //PB:
...
static void calc_sleep_time(tcpreplay_t *ctx, struct timespec *pkt_ts_delta,
        ...
        sendpacket_t *sp, COUNTER counter, struct timespec *sent_timestamp,
        COUNTER start_ns, COUNTER *skip_length) //PB:
{
    tcpreplay_opt_t *options = ctx->options;
    ...
    case speed_packetrate:
        /*
          ...
          */
         now_ns = TIMESPEC_TO_NANOSEC(sent_timestamp); //PB: INNEN FOLYTASD
         if (now_ns) {
             COUNTER next_tx_ns;
...
static void tcpr_sleep(tcpreplay_t *ctx, sendpacket_t *sp,
        struct timespec *nap_this_time, struct timespec *now) //PB:
{
    tcpreplay_opt_t *options = ctx->options;
    bool flush =
#ifdef HAVE_NETMAP
