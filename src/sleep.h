...
nanosleep_sleep(sendpacket_t *sp _U_, const struct timespec *nap,
        struct timespec *now,  bool flush _U_) // PB: timeval *now should be change to timespec *now
{
    nanosleep(nap, NULL);
#ifdef HAVE_NETMAP
    if (flush)
    ...
    clock_gettime(CLOCK_REALTIME, now); // PB: 
}
...
gettimeofday_sleep(sendpacket_t *sp _U_, struct timespec *nap,
                   struct timespec *now, bool flush _U_) //PB: check the whole function 
{
...
select_sleep(sendpacket_t *sp _U_, const struct timespec *nap,
        struct timespec *now_ns,  bool flush _U_) //PB:
{
    ...
    TIMEVAL_TO_TIMESPEC(&timeout, nap); //PB: it is just a bugfix
    ...
    struct timeval now_ms;
    gettimeofday(&now_ms, NULL);
    TIMEVAL_TO_TIMESPEC(&now_ms, now_ns); //PB:
}
#endif /* HAVE_SELECT */
...
void ioport_sleep(sendpacket_t *sp _U_, const struct timespec *nap,
        struct timespec *now,  bool flush); //PB:
